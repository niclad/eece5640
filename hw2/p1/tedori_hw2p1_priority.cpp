...
struct Philosopher {
    ...
    int fail;           // thread generation status
    bool priority;      // highest priority is true
};
...
void *philosophize(void *aPhilo) {
    ...
    while (RUNNING) {
        ...
        do {
            if (philo->priority) {
                msg = ">>Philosopher " + to_string(philo->id + 1) + " of " 
                    + to_string(NUM_PHILO) + " has decided to eat.\n";
                cout << msg;
...
*/
    ...
    for (int i = 0; i < numPhilosophers; i++) {
        ...
        // warn and exit with error is a mutex fails for initialize
        if (failed) {
    ...
    for (int i = 0; i < numPhilosophers; i++) {
        ...
        philo->priority = (i == 0) ? true : false;  // assign philosopher 0 highest priority
        //cout << philo->priority << endl;
    ...
    for (int i = 0; i < numPhilosophers; i++) {
        ...
        // warn and exit if a thread fails to join
