...
int main(int argc, char *argv[])
{
    ...
    MPI_Bcast(ROW_IDX, VERT, MPI_INT, 0, MPI_COMM_WORLD); // Boradcast row counts to all nodes

    // COUNT AUTHORS' CO-AUTHORS
    MPI_Barrier(MPI_COMM_WORLD); // ensure timing is fair
    start = MPI_Wtime();         // start timing

    countAuthors(tempAuth, numPerProc, authCount, rank); // count authors
